...
var KoshekhBot = function Constructor(settings) {
	...
	this.settings.name = this.settings.name || 'wtnv';
...
KoshekhBot.prototype._welcomeMessage = function() {
	this.postMessageToChannel('random','Words of wisdom from a friendly desert community where the sun is hot, the moon is beautiful, and mysterious lights pass overhead while we all pretend to sleep.\n \n Welcome to Night Vale.',{as_user:true});
};
...
KoshekhBot.prototype._replyWithRandomJoke = function(originalMessage) {
	this.db.get('SELECT id, quote FROM quotes ORDER BY used ASC, RANDOM() LIMIT 1', function (err, record) {
		if(err) {
			return console.error('DATABASE ERROR:'err);
		}
		
		var channel = this._getChannelById(originalMessage.channel);
		this.postMessageToChannel(channel.name, record.quote, {as_user:true});
		this.db.run('UPDATE quotes SET used = used + 1 WHERE id = ?',record.id);
	});
};
