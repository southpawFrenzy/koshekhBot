...
KoshekhBot.prototype._welcomeMessage = function() {
	...
	this.postMessageToChannel('random','Hello all! I will provide some words of wisdom from a friendly desert community where the sun is hot, the moon is beautiful, and mysterious lights pass overhead while we all pretend to sleep.\n \n Welcome... to Night Vale.\n \n Pssst, type !helpme here for more info', {as_user: true});
...
KoshekhBot.prototype._updateQuote = function() {
	...
	var nineOhClock = new Date(now.getFullYear(),now.getMonth(),now.getDate(),9,23,0,0) - now;
...
/*
 *Helper function to check if the object is directed to a channel or a direct message to koshekh.
 ...
 *when it starts with a "C" it represents a chat channel, and "D" represents an DM.
 */
...
KoshekhBot.prototype._searchForKeywords = function(message) {
	...
	if (message.text.toLowerCase().indexOf(this.name) > -1) {
		...
		self.postMessageToChannel(channel.name,'MEOW! \n \n Hello all! I will provide some words of wisdom from a friendly desert community where the sun is hot, the moon is beautiful, and mysterious lights pass overhead while we all pretend to sleep.\n \n Welcome... to Night Vale.\n \n Pssst, type !helpme here for more info', {as_user:true});
	}
	...
	if (message.text.toLowerCase().indexOf('!helpme') > -1) {
		
		self.postMessageToChannel(channel.name,"I am a helpful cat that hovers four feet off the ground in the mens bathroom of the Night Vale Community Radio station. I will provide a daily inspiring quote at 9:00 am from my compatriot, protector, and on-air host Cecil Baldwin.\n I can respond to a few basic commands as well. \n Currently I will respond to !whoareyou, !koshekh, !helpme, and !repeat.", {as_user:true});
	}
};
